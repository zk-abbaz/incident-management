...
class ProcessorService extends cds.ApplicationService {
    ...
    //function to check urgency code based on title of the incident
    changeUrgencyDueToSubject(data) {
    let urgent = data.title?.match(/urgent/i)
    if (urgent) data.urgency_code = 'H'
};

//custom validation to check if the incident is closed before allowing modifications
    async onUpdate (req) {
    let closed = await SELECT.one(1).from (req.subject).where `status.code = 'C'`
    if (closed) req.reject `Can't modify a closed incident!`
}

    //custom logic to check if the customer is blocked before allowing to read the details
...
//custom logic to cache the customer details in the local database before creating/updating an incident
